...
class CleanUpdatePoulesClubsCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        // Nouvelle logique : limite stricte de 4 clubs par poule pour A, B, C
        $maxClubsPerPoule = 4;
        $clubsCount = [
            $pouleA->getId() => 0,
            $pouleB->getId() => 0,
        ...
        foreach ($clubs as $club) {
            // Essayer de placer dans A, B, C si possible
            $placed = false;
            ...
            // Sinon, forcer dans la poule D
            if (!$placed) {
                $club->setPoule($pouleD);
                $this->entityManager->persist($club);
                $clubsCount[$pouleD->getId()]++;
            }
        }
        $this->entityManager->flush();
