...
class CleanUpdatePoulesClubsCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        // Nouvelle logique : limite stricte de 4 clubs par poule pour A, B, C, D
        $maxClubsPerPoule = 4;
        $clubsCount = [
            $pouleA->getId() => 0,
            $pouleB->getId() => 0,
        ...
        foreach ($clubs as $club) {
            // Essayer de placer dans A, B, C, D si possible
            $placed = false;
            foreach ([$pouleA, $pouleB, $pouleC, $pouleD] as $poule) {
                if ($clubsCount[$poule->getId()] < $maxClubsPerPoule) {
                    $club->setPoule($poule);
                    $this->entityManager->persist($club);
            ...
            // Si pas placé, ne rien faire (ou gérer selon besoin)
            if (!$placed) {
                // Ici on peut loguer ou gérer les clubs en trop si besoin
            }
        }
        $this->entityManager->flush();
