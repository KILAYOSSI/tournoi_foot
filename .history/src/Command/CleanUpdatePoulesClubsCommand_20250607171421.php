...
class CleanUpdatePoulesClubsCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        // Initialiser un tableau pour compter les clubs par poule
        $clubsCount = [
            $pouleA->getId() => 0,
            $pouleB->getId() => 0,
            $pouleC->getId() => 0,
        ...
        // Répartir les clubs dans les poules en limitant à 4 clubs par poule
        foreach ($clubs as $club) {
            $assigned = false;
            ...
            if (!$assigned) {
                // Tous les poules ont déjà 4 clubs, on ne peut pas assigner ce club
                $club->setPoule(null);
                $this->entityManager->persist($club);
            }
        }
