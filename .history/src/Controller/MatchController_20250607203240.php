...
class MatchController extends AbstractController
{
    ...
    public function matchDuJour(): Response
    {
        ...
        foreach ($classements as $classement) {
            ...
            // Assigner explicitement les clubs aux poules C et D selon les IDs
            if (in_array($clubId, $idsPouleC)) {
                $key = 'C';
            } elseif (in_array($clubId, $idsPouleD)) {
                $key = 'D';
            } else {
                $poule = $classement->getPoule();
                $pouleNom = $poule->getNom();
                if (in_array($pouleNom, ['Poule A', 'Poule B'])) {
