...
class MatchController extends AbstractController
{
     ...
     */
    public function matchDuJour(PouleRepository $pouleRepository, ClubRepository $clubRepository, \App\Repository\CartonRepository $cartonRepository): Response
    {
        ...
        // Récupérer les résultats des rencontres jouées
        ...
        foreach ($poulesEntities as $poule) {
            ...
            foreach ($rencontres as $rencontre) {
                ...
                foreach ($rencontre->getStatistiquematches() as $stat) {
            ...
            // Récupérer les cartons des deux côtés à partir de la table Carton
            $clubIds = array_map(fn($club) => $club->getId(), $clubs);
            $cartons = $cartonRepository->createQueryBuilder('c')
                ->where('c.joueur IN (
                    SELECT j.id FROM App\Entity\Joueur j WHERE j.club IN (:clubIds)
                )')
            ...
            foreach ($cartons as $carton) {
                $clubId = $carton->getJoueur()->getClub()->getId();
                if (in_array($clubId, $clubIds)) {
                    if ($carton->getType() === 'jaune') {
                        $classement[$clubId]['cartonsJaunes']++;
                    } elseif ($carton->getType() === 'rouge') {
                        $classement[$clubId]['cartonsRouges']++;
                    }
                }
            }
