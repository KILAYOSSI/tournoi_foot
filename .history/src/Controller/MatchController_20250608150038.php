    ...
    public function matchDuJour(PouleRepository $pouleRepository, ClubRepository $clubRepository): Response
    {
        ...
        foreach ($poulesEntities as $poule) {
            ...
            foreach ($rencontres as $rencontre) {
                ...
                // Mise à jour cartons (supposons méthodes getCartonsJaunesClub1(), getCartonsRougesClub1(), etc. existent)
                $classement[$club1->getId()]['cartonsJaunes'] += $rencontre->getCartonsJaunesClub1() ?? 0;
                $classement[$club1->getId()]['cartonsRouges'] += $rencontre->getCartonsRougesClub1() ?? 0;
                $classement[$club2->getId()]['cartonsJaunes'] += $rencontre->getCartonsJaunesClub2() ?? 0;
                $classement[$club2->getId()]['cartonsRouges'] += $rencontre->getCartonsRougesClub2() ?? 0;
