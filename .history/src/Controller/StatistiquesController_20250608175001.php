...
class StatistiquesController extends AbstractController
{
    ...
    public function index(Connection $connection): Response
    {
        ...
        $meilleursButeurs = $connection->fetchAllAssociative('
            SELECT j.nom, c.nom AS club, SUM(s.buts) AS buts, COUNT(s.match_id) AS matchs
        ...
        $meilleursPasseurs = $connection->fetchAllAssociative('
            SELECT j.nom, c.nom AS club, SUM(s.passes) AS passes, COUNT(s.match_id) AS matchs, SUM(s.minutes) AS minutes
            FROM statistiquematch s
            JOIN joueur j ON s.joueur_id = j.id
            JOIN club c ON j.club_id = c.id
            GROUP BY j.id, j.nom, c.nom
        ...
        // Requête pour les meilleurs gardiens
        $meilleursGardiens = $connection->fetchAllAssociative('
            SELECT j.nom, c.nom AS club, SUM(s.arrets) AS arrets, SUM(s.cleansheet) AS cleanSheets,
            (SUM(s.arrets) / NULLIF(SUM(s.tirs_encaisses), 0)) * 100 AS pourcentageArrets
            FROM statistiquematch s
            JOIN joueur j ON s.joueur_id = j.id
            JOIN club c ON j.club_id = c.id
            GROUP BY j.id, j.nom, c.nom
            ORDER BY pourcentageArrets DESC
            LIMIT 10
        ');
