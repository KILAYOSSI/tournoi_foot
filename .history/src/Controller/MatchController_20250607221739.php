...
class MatchController extends AbstractController
{
    ...
    public function matchDuJour(): Response
    {
        ...
        // Regrouper par poule
        $poules = ['A' => [], 'B' => [], 'C' => [], 'D' => []];
        // Suppression de la logique de forçage des clubs dans les poules C et D par ID
        foreach ($classements as $classement) {
            $poule = $classement->getPoule();
            if (!$poule) {
                continue; // Ignorer les classements sans poule
            }
            ...
            // Filtrer pour n'afficher que les poules C et D avec les clubs spécifiés
            $clubId = $classement->getClub()->getId();
            if ($key === 'C' && !in_array($clubId, [63, 59, 55, 51])) {
                continue;
            }
            if ($key === 'D' && !in_array($clubId, [64, 60, 56, 52])) {
                continue;
            }
