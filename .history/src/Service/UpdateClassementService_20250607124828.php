...
class UpdateClassementService
{
    ...
    {
        // Supprimer les anciens classements pour garder l'historique à jour
        $oldClassements = $this->classementPouleRepository->findAll();
        foreach ($oldClassements as $oldClassement) {
            $this->entityManager->remove($oldClassement);
        }
        ...
        foreach ($rencontres as $rencontre) {
            ...
            // Limiter à 6 matchs joués par club (commenté pour test)
            // if (count($stats[$club1Id]['matchsJouesIds']) >= 6) {
            //     continue;
            // }
            // if (count($stats[$club2Id]['matchsJouesIds']) >= 6) {
            //     continue;
            // }
        ...
        // Enregistrer les classements avec rang par poule
        foreach ($statsParPoule as $pouleId => $statsPoule) {
            $rang = 1;
            foreach ($statsPoule as $stat) {
                $classement = new ClassementPoule();
                $classement->setClub($stat['club']);
                $classement->setPoints($stat['points']);
                $classement->setButsPour($stat['butsPour']);
                $classement->setButscontre($stat['butscontre']);
                $classement->setGoalaverage($stat['goalaverage']);
                $classement->setMatchsjouer($stat['matchsjouer']);
                $classement->setGagnes($stat['gagnes']);
                $classement->setNuls($stat['nuls']);
                $classement->setPerdus($stat['perdus']);
                // Ajout de la poule dans le classement
                $classement->setPoule($this->entityManager->getReference(\App\Entity\Poule::class, $pouleId));
