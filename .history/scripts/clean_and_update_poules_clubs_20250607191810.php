...
$clubsCountPerPoule = [0, 0, 0, count($clubsPouleD)];
foreach ($clubs as $club) {
    $poule = $club->getPoule();
    if ($poule) {
...
foreach ($clubs as $club) {
    if ($club->getPoule() === null) {
        // Trouver une poule avec moins de 4 clubs, sauf la poule D
        $assigned = false;
        for ($i = 0; $i < 3; $i++) {
        ...
        // Si toutes les poules A, B, C ont 4 clubs, assigner à la poule D
        if (!$assigned) {
            $club->setPoule($pouleD);
            $entityManager->persist($club);
