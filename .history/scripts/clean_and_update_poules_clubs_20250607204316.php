...
$forcedClubsPouleDIds = [64, 60, 56, 52];
$forcedClubsPouleCIds = [63, 59, 55, 51];
...
for ($i = 0; $i < 3; $i++) { // Pour les poules A, B, C
    while ($clubsCountPerPoule[$i] > 4) {
        ...
        foreach ($clubsInPoule as $club) {
            ...
            // Ne pas déplacer les clubs forcés dans la poule D ou C
            $clubId = $club->getId();
            if (in_array($clubId, $forcedClubsPouleDIds) || in_array($clubId, $forcedClubsPouleCIds)) {
                continue;
            }
            $club->setPoule($pouleD);
            $entityManager->persist($club);
